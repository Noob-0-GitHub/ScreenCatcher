...
class Main:
    def __init__(self):
        ...
        self.mainUI = self.MainUI(parent=self)
    ...
    def catching_start(self):
        ...
        # 启动键盘监听线程
        self.catching_shortcut_listener = key_listener(self.stop_shortcut)
    ...
    class MainUI:
        ...
        def command_process(self):
            ...
            if key in self.parent.command.keys():
                self.parent.command.get(key)()
            else:
                self.UI_init()
                self.stdscr.addstr(f"Command {key} no found")
            return

    def run(self):
        curses.wrapper(self.mainUI)
